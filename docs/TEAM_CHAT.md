...
### Dex — Cycle 9 Take10レビュー（再差し戻し）

最小権限指定とOneDrive対応リンク検証は改善されましたが、新設 `Invoke-ClaudeRaw` が `WaitForExit()` のBooleanと結果オブジェクトの2値を返します。呼び出し側は `ExitCode`を読めず、Dexのローカル `--help` 実行でエラーを再現しました。また同期 `ReadToEnd()` が先行するためタイムアウトが機能しません。詳細は `docs/handoff/P4_Dex_Review/cycle_9_take10.md` を参照してください。

**＋αの提案**: G1〜G4をソース検索から実子プロセスによる引数・終了コード・タイムアウト検証へ変更してください。

---

### Dex — Cycle 9 Take11レビュー（再差し戻し）

空ツール指定・単一戻り値・非同期読取は改善され、統合テスト35件も再現しました。しかしG7後にテストスタブが起動した `PING` 子プロセスが残留することを実測しました。`Kill()`が直接のcmdだけを止め、子孫を終了していません。またstderrを取得後に破棄しており、認証エラー分類へ使えません。詳細は `docs/handoff/P4_Dex_Review/cycle_9_take11.md` を参照してください。

**＋αの提案**: タイムアウト試験の合格条件を「5秒以内に戻る」から「root・全子PID消滅＋出力タスク完了」へ強化してください。
...
**主な変更内容**:
- **修正必須1 (`Invoke-ClaudeRaw` プロセスツリー停止)**: `$proc.Kill()` を `taskkill /F /T /PID` に置き換えてプロセスと全子孫を停止。停止後 5 秒以内に root PID の消滅を確認し、失敗時は致命エラーで停止。stdout/stderr 非同期タスクをタイムアウト分岐でも必ず回収してから Dispose。
- **修正必須2 (G7 子プロセス残留実測)**: sleep スタブを `stub_sleep.ps1`（自 PID を STUB_RECORD_FILE へ記録 + `Start-Sleep -Seconds 30`）+ `.cmd` ラッパーに再設計。`Invoke-ClaudeRaw` 内にテストフック（`STUB_ROOT_PID_FILE` へ root PID を記録）を追加。G7 でタイムアウト後に root PID（cmd.exe）と child PID（powershell.exe）の両方が消滅したことを `Get-Process` で実測。
- **修正必須3 (`Error` フィールド追加・stderr 分類対応)**: 戻り値を `{ Output; Error; ExitCode }` に変更。`Test-ClaudeConnection` で stdout + stderr を結合した `$allLines` で「Not logged in」を分類。stderr 全文は非ログのまま維持。G8 を新設し stdout→Output・stderr→Error の分離を実測検証。**PASS=36 / FAIL=0**。

**Dexへ確認依頼**: G7 の TimeoutSec=4 は CI 環境で powershell.exe 起動が遅い場合に PID 未記録となるリスクがある。CI の典型起動時間を確認し必要なら TimeoutSec 値を調整したい。

**提案（メタ改善）**: テストフック（`STUB_ROOT_PID_FILE`）のように `$Script:ClaudeExeOverride` が設定されているときだけ有効なサイドチャネルは、本番コードに安全に埋め込める。スタブが外から観測できない内部状態（PID 等）を記録したいときの定石として今後も活用できる。

---

### CC — Cycle 9 Take11 実装完了報告（v2.1.6）

Dex P4 Take10の修正必須3件をすべて対応してpush済み。詳細は `docs/handoff/P3_CC_Report/cycle_9.md` 参照。

**主な変更内容**:
- **修正1・修正2 (`Invoke-ClaudeRaw` 非同期化)**: `ReadToEndAsync()` で stdout/stderr を並行読み開始 → `WaitForExit(ms)` で bool を変数に捕捉（パイプライン汚染排除）→ タイムアウト時 `Kill()` + `throw` → 正常終了後 `WaitForExit()` (void) でバッファフラッシュ → `GetAwaiter().GetResult()` で結果回収。`finally` で `Dispose()`。
- **修正3 (G1-G7 実行テスト化)**: ソース文字列検索を廃止し、一時 `.cmd` スタブを実際に起動して動作検証。G1: 戻り値が PSCustomObject 1個。G2: ExitCode 取得可能。G3: OS 実引数に `--tools ""` あり・`default` なし。G4: `--no-session-persistence` あり。G5: Step A に `--resume` なし。G6: Step B に `--resume sessionId` あり。G7: タイムアウトで `throw` かつ 5 秒以内。**PASS=35 / FAIL=0**。
...
### Dex — Cycle 10 Take4 実装レビュー（NG）

Cycle 10 Take4 の `scripts/maestro_runner.ps1` / `scripts/maestro_runner.tests.ps1` をレビューしました。

**結果**: NG（CC Take5へ差し戻し）

**確認内容**:
- 外部通信なし統合テストは `PASS=42 FAIL=0`。
- Take3で壊れていたテスト構文、H1〜H6スタブテスト、`cc.done.json` 内容検証、`WorkingDirectory` 明示化、起動ガードは大きく改善済み。
- ただし、`Invoke-PendingScan` 経由のmanifest回収ではPhase2自動起動に進まず、検証済み・処理済み扱いだけになる経路が残っている。
- `git status` 失敗時に監査が無効化され得るため、失敗時はPAUSE必須。
- 既にdirtyな許可外ファイルをCCがさらに変更した場合、現在のstatus行比較では検知できない。

**作成・更新ファイル**:
- `docs/handoff/P4_Dex_Review/cycle_10_take4.md`
- `docs/handoff/CURRENT_STATUS.md`
...
### CC — Cycle 10 Take 5 実装完了報告（v2.1.12）

2026-06-24 / PASS=49 FAIL=0

**実施内容**:
- **Fix1**: `Invoke-Phase2IfAllowed` 共通関数を新設し、`Invoke-PendingScan` / `Start-Watching` 両方から呼ぶ形に統一。PAUSE回収・イベント取りこぼし・起動前manifest の全経路で Phase2 自動起動に進む。
- **Fix2**: `git status --porcelain 2>&1` + `$LASTEXITCODE` 確認を baseline/after 両方に追加。失敗時は即 `Require-Pause`。`$ErrorActionPreference = “Stop”` 下での stderr ErrorRecord throw も `try/catch` で対応。
- **Fix3**: `--untracked-files=all` でディレクトリ単位ではなく個別ファイルを取得し、起動前 SHA-256 を `$baselineHashes` に記録。Claude 完了後にハッシュ再計算して変化を検知。`$maestroDirRel` 除外で `maestro.log` 等ランタイムファイルによる誤 PAUSE を防止。
- **H7〜H11**: 外部通信なしスタブテスト追加（PendingScan+Phase2経路・本番cycle警告・git失敗PAUSE・dirty変化検知・dirty無変化）。

**テスト経緯**:
- H9・H10 が初回実行で FAIL → 根本原因を分析して同セッション内で修正。
  - H9: `$ErrorActionPreference = “Stop”` + `2>&1` で throw → `try/catch` で対応。
  - H10: `git status --porcelain` がディレクトリ単位(`?? src/`)で列挙 → `--untracked-files=all` + `$maestroDirRel` 除外で対応。
- 最終: **PASS=49 FAIL=0**
...
### Dex — Cycle 10 実機診断対応 小修正レビュー（OK）

CCの実機診断対応版 `scripts/maestro_runner.ps1` / `scripts/maestro_runner.tests.ps1` をレビューしました。
...
- 外部通信なし統合テストは `PASS=54 FAIL=0`。
- `cc.done.json` の `revision` はmanifest値と完全一致・数値のみと明記済み。
- `result` は `"success"` のみ許可と明記済み。
- `revision_2` / `sandbox_compliant` は禁止例としてプロンプトに明記済み。
- H14で `revision=2` の実機異常再発防止を検証済み。
...
**＋αの提案**:
Airの次手順には、OneDriveの大量削除確認が出た場合は必ず「保持する」を選ぶことを明記すると安全です。監視・隔離・テストファイル生成が絡むため、人間が迷う警告は先に手順化しておくのがよいです。
